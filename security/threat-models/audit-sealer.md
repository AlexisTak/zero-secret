...
| **S**poofing | Un `access-broker`/`credential-issuer` compromis (ou tout appelant légitime du réseau normal vers `audit-collector`) fait sceller un `policy.decided`/`credential.issued` portant un `decision_hash`/`reasons` fabriqué, jamais réellement produit par le PDP (ADR-027/029) | Faible si le producteur n'est pas compromis, mais structurellement possible : `audit-seal/v1` signe ce qu'on lui donne | Élevé (le journal revendique une décision qui n'a jamais eu lieu) | `decision.decision_signature`/`decision_signature_key_id` (recopiés de `decision-seal/v1`) sont transportés dans l'événement scellé — un vérificateur hors ligne possédant la clé publique `zs-decision-seal-v1` peut détecter une incohérence | **`audit-sealer` ne vérifie PAS `decision_signature` au scellement** (décision délibérée, ADR-027 : un contrôle à l'exécution ne laisse aucune trace pour un vérificateur futur, et ajouterait une dépendance réseau sortante sur le chemin de scellement) — `audit-seal/v1` atteste la détention du `decision_hash`, pas son origine PDP, tant qu'aucun vérificateur hors ligne n'existe dans ce dépôt pour exploiter `decision_signature` |
